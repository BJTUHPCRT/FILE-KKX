...
from entities import environment_pretrain
...
def launcher(environment):
    ...
    def running_machine():
        ...
        global timer
        timer = threading.Timer(2, running_machine)  
        timer.start()
    running_machine()
    ...
    for episode in range(200):
        ...
        while True:
            ...
            if (step <= random_counter):
                action = random_agent.choose_action()

            # RL take action and get next observation and reward
            observation_, reward, done, workload_changed, fluction_index = environment.step(action)
            ...
            # if fluction_index != -1:
            #     print(fluction_index, gamma_tmp, environment.task_allocate_counter,episode)
...
def  DQNrl(task_temp_length,task_temp, step, environment, observation, gamma_list):
    task_count = 0
    current_step = step
    ...
    while (task_count < task_temp_length):
        ...
        if (step > 360) and (step % 5 == 0) and (task_count == 1):
            DQN.learn() 
            gamma_list.append(DQN.gamma)
        if (step > 360) and (step % 5 == 0):
            DQN.learn() 
            gamma_list.append(DQN.gamma)

        if (step - current_step) > 30 and DQN.gamma < 0.9:
            DQN.gamma += 0.1
...
def plot_reward(reward_averge9,reward_averge8,reward_averge7,reward_averge6,reward_averge5,reward_averge4,reward_averge3,reward_averge2,reward_averge1,reward_averge0,min_reward,max_reward,iterations, start, episode):
    ...
    plt.title('Compare reward different gamma-' + start + '-' + episode)  # 标题
...
if __name__ == "__main__":
    # maze game
    global environment
    environment = environment_pretrain.environment()
